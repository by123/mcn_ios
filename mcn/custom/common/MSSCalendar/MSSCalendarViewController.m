...
- (void)initDataSource
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        MSSCalendarManager *manager = [[MSSCalendarManager alloc]initWithShowChineseHoliday:weakSelf.showChineseHoliday showChineseCalendar:weakSelf.showChineseCalendar startDate:weakSelf.startDate];
        NSArray *tempDataArray = [manager getCalendarDataSoruceWithLimitMonth:weakSelf.limitMonth type:weakSelf.type];
...
- (void)addWeakView
{
    _weekView = [[UIView alloc]initWithFrame:CGRectMake(0, StatuBarHeight, MSS_SCREEN_WIDTH, MSS_WeekViewHeight)];
    _weekView.backgroundColor = MSS_SelectBackgroundColor;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(cell)
    {
        ...
        //今天日期
        else if(calendarItem.dateInterval == [STTimeUtil getTimeStamp:[STTimeUtil generateDate:[STTimeUtil getCurrentTimeStamp] format:MSG_DATE_FORMAT] format:MSG_DATE_FORMAT]){
            cell.dateLabel.textColor = c16;
            cell.subLabel.text = @"今天";
        }
        // 开始和结束之间的日期
        ...
        else
        {
            ...
            if(calendarItem.holiday.length > 0 && _showHolidayDifferent)
            {
