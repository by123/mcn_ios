...
@interface CelebrityListViewModel()

@property(assign, nonatomic)int currentPage;

@end

@implementation CelebrityListViewModel : NSObject
...
-(void)requestList:(Boolean)isRefreshNew{
    if(!_delegate) return;
     ...
     WS(weakSelf)
     [STNetUtil post:URL_MCH_CELEBRITY_LIST content:dic.mj_JSONString success:^(RespondModel *respondModel) {
         if([respondModel.status isEqualToString:STATU_SUCCESS]){
             ...
             NSMutableArray *requestDatas = [CelebrityModel mj_objectArrayWithKeyValuesArray:datas];
...
-(void)reqeustInviteList:(Boolean)isRefreshNew{
    ...
    dic[@"page"] = @(_currentPage);
    dic[@"size"] = @(XW_PAGESIZE);
    dic[@"inviteType"] = @(_inviteType);
    WS(weakSelf)
    [STNetUtil post:URL_MCHINVITE_CELEBRITY_LIST content:dic.mj_JSONString success:^(RespondModel *respondModel) {
        if([respondModel.status isEqualToString:STATU_SUCCESS]){
            id data = respondModel.data;
            int pages = [[data objectForKey:@"pages"] intValue];
            int current = [[data objectForKey:@"current"] intValue];
