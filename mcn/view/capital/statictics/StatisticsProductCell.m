...
@interface StatisticsProductCell()
...
@implementation StatisticsProductCell
...
-(void)updateData:(StatisticsProductModel *)model hiddenLine:(Boolean)hiddenLine{
    ...
    _totalLabel.text = [NSString stringWithFormat:@"%.2f",model.total / 100];
...
+(NSString *)identify{
    return NSStringFromClass([StatisticsProductCell class]);
}
